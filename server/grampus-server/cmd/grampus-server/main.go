...
import (
	...
	"server/grampus-server/internal/server"
	"server/grampus-server/internal/service"
...
func main() {
	...
	app, err := initApp(context.Background(), conf, log.DefaultLogger)
	if err != nil {
		panic(err)
	}
...
func initApp(ctx context.Context, bc *conf.Bootstrap, logger log.Logger) (*kratos.App, error) {
	...
	app := newApp(ctx, logger, httpServer)
	return app, nil
}
